...
1. Fetch `GET http://app.clawvisor.com/api/skill/version` — it returns:
   ```json
   { "skill_version": "...", "skill_published_at": "..." }
   ```
2. Compare the `skill_version` in the response with the version in this skill's frontmatter (``, published dev).
3. If a newer version is available, re-fetch the skill from `http://app.clawvisor.com/skill/SKILL.md` to get the latest instructions.
