---
name: ship
description: Commit pending work, push it, and merge to main if needed. Use when Robbie asks to ship, commit, push, or merge finished changes.
---
...
2. Stage selectively. Read the diff. Run `git add <specific files>` for only what you changed this session. Skip unrelated noise such as caches, IDE files, settings drift you did not author, and anything you cannot explain. When in doubt, ask.

3. Commit. Match the repo's recent commit style from `git log --oneline -5`. Lead with what changed and why.
...
6. Merge to main. If the current branch is not `main` or `master`:
   - Ask whether to merge, since some feature branches should stay open.
   - If yes, switch to main, pull, fast-forward merge if possible, otherwise use `--no-ff`, then push.
